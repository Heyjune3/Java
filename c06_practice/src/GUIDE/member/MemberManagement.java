...
public class MemberManagement{
	...
	void join() {
		...
		String mId = getData("아이디를 입력해주세요 > ");
		String mPw = getData("비밀번호를 입력해주세요 > ");
		String mPwC = getData("비밀번호를 확인해주세요 > ");
		if(mPwC != mPw) {
			System.out.println("이미 사용 중이거나 비밀번호가 일치하지 않습니다.");
			return;
		}
		String mName = getData("이름을 입력해주세요 > ");
		for(int i = 0; i < members.length-1; i++) {
			if(members[i] == null) {
				members[i] = new Member(count++,mId,mPw,mName);
