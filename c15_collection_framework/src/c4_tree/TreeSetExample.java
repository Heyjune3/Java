...
public class TreeSetExample {
	...
	public static void main(String[] args) {
		...
		System.out.println(newSet);  // [75, 80]
		
		// tailSet(81) 81 이후 값으로 새로운 set 생성
		newSet = treeSet.tailSet(81);
		System.out.println(newSet);  // [87, 90]
		
		// subSet(81,88) - 81 이후 88 이전의 범위셋 값으로 새로운 set 생성
		newSet = treeSet.subSet(81, 88);
		System.out.println(newSet); // [87]
		
		// 역순 정렬된 반복자 제공 Ascending - 오름차순, Desending - 내림차순
		Iterator<Integer> itr = treeSet.descendingIterator();
		while(itr.hasNext()) {
			int i = itr.next();
			System.out.print(i+" ");  // 90 87 80 75 
		}
		System.out.println();
		
		// 역순으로 정렬된 set 반환
		Set<Integer> descSet = treeSet.descendingSet();
		System.out.println(descSet);  // [90, 87, 80, 75]
	}
